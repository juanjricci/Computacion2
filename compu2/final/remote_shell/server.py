...
while True:
    ...
    legajo = clientsocket.recv(1024).decode()
    #clientsocket.send(msg.encode('ascii'))

    cur = conn.cursor()
    cur.execute(f"SELECT * FROM polls_alumnos where id = {legajo}")
    ...
    alumno = cur.fetchone()
    print(alumno)
    codigo_carrera = alumno[3]
    print(f'Hola {alumno[1]} {alumno[2]}!')

    
    clientsocket.send(bytearray(str(alumno).encode()))
