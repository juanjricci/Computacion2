...
static int Base64Encode(const char* message, char** buffer) { //Encodes a string to base64
  BIO *bio, *b64;
  FILE* stream;
  int encodedSize = 4*ceil(((double)strlen(message))/3);
  *buffer = (char *)malloc(encodedSize+1);

  stream = fmemopen(*buffer, encodedSize, "w");
...
static int Base64Decode(char* b64message, char** buffer) { //Decodes a base64 encoded string
  ...
  *buffer = (char*)malloc(decodeLen+1);
  FILE* stream = fmemopen(b64message, strlen(b64message)+1, "r");
...
int main(int argc, const char *argv[]){
	...
	char msg[256]="Hola Mundo, que tal";
	Base64Encode(msg,&buff1);
	printf("Codeado: %s\n", buff1);
	Base64Decode(buff1,&buff2);
	printf("Decodeado: %s\n", buff2);
